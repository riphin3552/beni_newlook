...
/// Modèle pour les espaces et chambres occupés
class EspaceChambre {
  final int idEspace;
...
/// Récupérer la liste des espaces occupés via l'API
Future<List<EspaceChambre>> fetchEspacesOccupes(int idEse) async {
  //print("DEBUG (Espaces API) - Sending Id_Ese: $idEse, statut: Occupé");
  final response = await http.post(
    Uri.parse("https://riphin-salemanager.com/beni_newlook_API/RapportEspacesChambresOccupes.php"),
    headers: {"Content-Type": "application/json"},
    body: jsonEncode({
...
Future<pw.Document> buildEspacesOccupesPdf(int idEse) async {
  ...
  pdf.addPage(
    pw.MultiPage(
      ...
      build: (context) => [
        ...
        pw.Center(
          child: pw.Text("Rapport des Espaces & Chambres Occupés",
...
class EspacesOccupesReportPage extends StatelessWidget {
  final int idEse;
  const EspacesOccupesReportPage({super.key, required this.idEse});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("Rapport Occupations")),
      body: PdfPreview(build: (format) async => (await buildEspacesOccupesPdf(idEse)).save()),
    );
  }
}